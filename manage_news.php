...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_news'])) {
        ...
        if (isset($_FILES['news_image']) && $_FILES['news_image']['error'] === 0) {
            ...
            if (in_array($file_ext, $allowed)) {
                ...
                if (move_uploaded_file($_FILES['news_image']['tmp_name'], $upload_dir . $new_filename)) {
        ...
        if (empty($msg_type)) { 
            $sql = "INSERT INTO announcements (title, content, image_path, is_active) VALUES ('$title', '$content', '$image_path', 1)";
    ...
    elseif (isset($_POST['delete_id'])) {
        ...
        $query = mysqli_query($conn, "SELECT image_path FROM announcements WHERE id = $id");
...
<head>
    ...
    <title>Gallery Updates | ManCave Admin</title>
    ...
    <style>
        /* =========================================
           INLINED ADMIN THEME (Matches admin.php)
           ========================================= */
        :root {
            /* Base Colors */
            --bg-body: #f8f9fc;
        ...
        * { box-sizing: border-box; }
        ...
        /* === SPECIFIC NEWS MANAGER STYLES === */
        .preview-img { width: 60px; height: 60px; border-radius: 8px; object-fit: cover; border: 1px solid var(--border-color); }
...
<body>
    ...
    <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
            <ul>
                ...
                <li class="active"><a href="manage_news.php"><i class="fas fa-newspaper"></i> <span>Gallery Updates</span></a></li>
    ...
    <main class="main-content">
        ...
        <header class="top-bar">
            <div class="page-header">
                ...
                <p>Post news and exhibitions to the Newsfeed page.</p>
            </div>
                        ...
                        <table class="styled-table">
                            ...
                            <tbody>
                                <?php if(empty($announcements)): ?>
                                    <tr><td colspan="4" class="text-center" style="padding: 30px; color: var(--text-muted);">No updates found. Post your first one!</td></tr>
                                <?php else: foreach($announcements as $row): ?>
                                <tr>
                                    <td>
                                        <?php if($row['image_path']): ?>
                                            <img src="uploads/<?php echo htmlspecialchars($row['image_path']); ?>" class="preview-img" alt="News Image">
                                        <?php else: ?>
                                            <div class="preview-img" style="background: var(--bg-body); display: flex; align-items: center; justify-content: center; color: var(--text-muted);">
                                    ...
                                    <td>
                                        <label class="toggle-switch">
                                            <input type="checkbox" onchange="toggleStatus(<?php echo $row['id']; ?>)" <?php echo $row['is_active'] ? 'checked' : ''; ?>>
                                            <span class="slider"></span>
                                        </label>
                                    </td>
                                    <td>
                                        <div style="margin-bottom: 4px;">
                                            <strong style="font-size:1rem; color:var(--text-main);"><?php echo htmlspecialchars($row['title']); ?></strong>
                                            <span style="font-size:0.75rem; color:var(--text-muted); margin-left: 8px;"><i class="far fa-clock"></i> <?php echo date('M d, Y', strtotime($row['created_at'])); ?></span>
                                        </div>
                                        <p style="margin:0; font-size:0.9rem; color:var(--text-muted); line-height:1.5;">
                                            <?php echo substr(strip_tags($row['content']), 0, 100) . (strlen($row['content']) > 100 ? '...' : ''); ?>
                                        </p>
                                    </td>
                                    <td>
                                        <form method="POST" onsubmit="return confirm('Permanently delete this update?');">
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        <div class="form-group">
                            ...
                            <div class="file-upload-wrapper">
                                <input type="file" name="news_image" accept="image/*" onchange="previewFile(this)">
                                <i class="fas fa-cloud-upload-alt file-upload-icon"></i>
                                <span class="file-upload-text" id="fileLabel">Click to Upload Image</span>
                            </div>
                        </div>

                        <div class="form-group">
                            <label>Title / Headline</label>
                            <input type="text" name="title" required placeholder="e.g. Grand Opening Exhibition">
                        </div>

                        <div class="form-group">
                            <label>Content Details</label>
                            <textarea name="content" rows="6" required placeholder="Write the details here..."></textarea>
                        </div>

                        <button type="submit" name="add_news" class="btn-primary">
    ...
    <script>
        ...
        // --- GLOBAL ADMIN SCRIPTS (Identical to admin.php) ---
