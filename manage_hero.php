...
if (isset($_POST['delete_id'])) {
    ...
    $stmt = $conn->prepare("SELECT image_path FROM hero_slides WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $res = $stmt->get_result();
    if ($row = $res->fetch_assoc()) {
        $file_path = "uploads/hero/" . $row['image_path'];
...
// Fetch Slides
$slides = [];
$res = mysqli_query($conn, "SELECT * FROM hero_slides ORDER BY id DESC");
...
<head>
    ...
    <title>Manage Slider | ManCave Admin</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .slide-item {
            ...
            background: var(--bg-body); box-shadow: 0 4px 10px var(--shadow-color);
            transition: 0.3s;
        }
        ...
        .delete-btn {
            width: 45px; height: 45px; border-radius: 50%; background: #ff4d4d;
            color: white; border: none; display: flex; align-items: center; justify-content: center;
            font-size: 1.2rem; cursor: pointer; transition: 0.2s; box-shadow: 0 5px 15px rgba(0,0,0,0.3);
        }
        .delete-btn:hover { transform: scale(1.1); background: #d63031; }
        ...
        .file-upload-wrapper {
            ...
            background: var(--bg-body); transition: 0.3s; cursor: pointer; margin-bottom: 20px;
        }
    ...
    <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
            <ul>
                ...
                <li class="active"><a href="manage_hero.php"><i class="fas fa-images"></i> <span>Manage Slider</span></a></li>
    ...
    <main class="main-content">
        ...
        <header class="top-bar">
            <div class="page-header">
                <h1>Manage Slider</h1>
                <p>Upload and manage the rotating background images for the homepage.</p>
            </div>
                ...
                <div class="card">
                    ...
                    <?php if(empty($slides)): ?>
                        <div style="text-align: center; padding: 40px; color: var(--text-muted);">
                            <i class="far fa-images" style="font-size: 3rem; margin-bottom: 10px;"></i>
                    ...
                    <?php else: ?>
                        <div class="slide-grid">
                            ...
                            <div class="slide-item">
                                <img src="uploads/hero/<?php echo htmlspecialchars($slide['image_path']); ?>" alt="Slide">
                                <div class="slide-overlay">
                                    <form method="POST" onsubmit="return confirm('Delete this slide?');">
                                        <input type="hidden" name="delete_id" value="<?php echo $slide['id']; ?>">
                                        <button type="submit" class="delete-btn" title="Delete Slide"><i class="fas fa-trash-alt"></i></button>
                                    </form>
                                </div>
                            </div>
                            <?php endforeach; ?>
                ...
                <div class="card">
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        <div class="file-upload-wrapper">
                            <input type="file" name="hero_image" accept="image/*" onchange="previewFile(this)" required>
                            <i class="fas fa-cloud-upload-alt file-upload-icon"></i>
                            <span class="file-upload-text" id="fileLabel">Click to Upload Image</span>
                        </div>
                        <p style="font-size: 0.8rem; color: var(--text-muted); margin-bottom: 20px; text-align: center;">Recommended Size: 1920x1080 (JPG, PNG)</p>
                        <button type="submit" name="upload_image" class="btn-primary">
                            <i class="fas fa-upload"></i> Upload to Slider
                        </button>
                    </form>
                </div>
            </div>
