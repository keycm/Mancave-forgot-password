...
if (isset($_POST['add_member'])) {
    ...
    if (in_array($imageFileType, $allowed)) {
        if (move_uploaded_file($_FILES["profile_image"]["tmp_name"], $target_file)) {
            $stmt = $conn->prepare("INSERT INTO about_artists (name, role, image_path) VALUES (?, ?, ?)");
            $stmt->bind_param("sss", $name, $role, $file_name);
            $stmt->execute();
...
if (isset($_POST['delete_id'])) {
    ...
    $stmt = $conn->prepare("SELECT image_path FROM about_artists WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $res = $stmt->get_result();
    if ($row = $res->fetch_assoc()) {
        $file_path = "uploads/team/" . $row['image_path'];
        if (file_exists($file_path)) { unlink($file_path); }
        $del = $conn->prepare("DELETE FROM about_artists WHERE id = ?");
        $del->bind_param("i", $id);
        $del->execute();
...
// Fetch Members
$members = [];
$res = mysqli_query($conn, "SELECT * FROM about_artists ORDER BY id DESC");
...
<head>
    ...
    <title>Manage Artists | ManCave Admin</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .form-control:focus { outline: none; border-color: var(--accent); box-shadow: 0 0 0 3px rgba(205, 133, 63, 0.1); }
        ...
        .member-img { width: 100%; height: 250px; object-fit: cover; border-bottom: 1px solid var(--border-color); }
        .member-info { padding: 15px; }
        .member-name { font-weight: 700; font-size: 1.1rem; color: var(--text-main); margin-bottom: 5px; }
        .member-role { color: var(--accent); font-size: 0.85rem; text-transform: uppercase; font-weight: 700; letter-spacing: 1px; }
        ...
        .delete-btn { 
            ...
            box-shadow: 0 4px 10px rgba(0,0,0,0.2); transition: 0.2s; opacity: 0; transform: translateY(-10px);
        }
        .member-card:hover .delete-btn { opacity: 1; transform: translateY(0); }
        .delete-btn:hover { background: #d63031; transform: scale(1.1); }

        /* Alert */
        .alert-box {
            padding: 15px 20px; border-radius: var(--radius-soft); margin-bottom: 25px;
            color: #fff; display: flex; align-items: center; gap: 10px; font-weight: 600;
    ...
    <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
            <ul>
                ...
                <li class="active"><a href="manage_team.php"><i class="fas fa-user-tie"></i> <span>Manage Artists</span></a></li>
    ...
    <main class="main-content">
        <header class="top-bar">
            <div class="page-header">
                ...
                <p>Add or remove artists featured on the website.</p>
            </div>
                ...
                <div class="card">
                    ...
                    <?php if(empty($members)): ?>
                        <div style="text-align: center; padding: 40px; color: var(--text-muted);">
                            <i class="fas fa-user-friends" style="font-size: 3rem; margin-bottom: 10px;"></i>
                    ...
                    <?php else: ?>
                        <div class="member-grid">
                            ...
                            <div class="member-card">
                                <img src="uploads/team/<?php echo htmlspecialchars($m['image_path']); ?>" alt="<?php echo htmlspecialchars($m['name']); ?>" class="member-img">
                                <div class="member-info">
                                    <div class="member-name"><?php echo htmlspecialchars($m['name']); ?></div>
                                    <div class="member-role"><?php echo htmlspecialchars($m['role']); ?></div>
                                </div>
                                <form method="POST" onsubmit="return confirm('Remove this artist?');">
                ...
                <div class="card">
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="name" class="form-control" required placeholder="e.g. Leonardo Da Vinci">
                        </div>

                        <div class="form-group">
                            <label>Role / Style</label>
                            <input type="text" name="role" class="form-control" required placeholder="e.g. Abstract Expressionist">
                        </div>
