...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .modal-card.small { width: 400px; max-width: 90%; border-radius: 16px; padding: 35px; text-align: center; transform: translateY(20px); transition: 0.3s; }
        ...
        .modal-header h3 { font-family: var(--font-head); font-size: 1.5rem; margin: 0; }
        ...
        .form-group input, .form-group select { width: 100%; padding: 12px 15px; border: 1px solid var(--border-color); border-radius: 8px; font-size: 0.95rem; background: var(--input-bg); color: var(--text-main); font-family: var(--font-main); }
        .btn-friendly { padding: 12px 25px; border-radius: 8px; font-weight: 700; border: none; cursor: pointer; transition: 0.2s; font-family: var(--font-main); }
...
<body>
                    ...
                    <tbody>
                        ...
                        <?php else: ?>
                            <?php foreach ($users as $user): 
                                ...
                                $roleClass = 'role-user'; $roleLabel = 'Collector';
                                if ($role === 'admin') { $roleClass = 'role-admin'; $roleLabel = 'Admin'; }
                                elseif ($role === 'manager') { $roleClass = 'role-manager'; $roleLabel = 'Manager'; }
                                $initial = strtoupper(substr($user['username'], 0, 1));
                            ?>
                            <tr data-id="<?= $user['id'] ?>" data-username="<?= htmlspecialchars($user['username']) ?>" data-email="<?= htmlspecialchars($user['email']) ?>" data-role="<?= htmlspecialchars($role) ?>">
                                ...
                                <td style="text-align: right;">
                                    <div class="actions">
                                        <button class="btn-icon edit" onclick="editUser(this)"><i class="fas fa-pen"></i></button>
                                        <?php if ($user['id'] != ($_SESSION['user_id'] ?? 0)): ?>
                                            <button class="btn-icon delete" onclick="confirmDelete(<?= $user['id'] ?>)"><i class="fas fa-trash"></i></button>
                                        <?php endif; ?>
                                    </div>
                                </td>
                            </tr>
    ...
    <div class="modal-overlay" id="userModal">
        <div class="modal-card form-modal">
            <div class="modal-header">
                ...
                <button class="small-btn" onclick="closeModal('userModal')" style="font-size:1.5rem; line-height:1;">&times;</button>
            </div>
            <form id="userForm">
                ...
                <div class="form-group"><label>Password</label><input type="password" id="password" name="password"><small style="color:var(--text-muted); display:block; margin-top:5px;">Min 8 chars. Blank to keep current.</small></div>
                <div class="form-group"><label>Role</label><select id="role" name="role"><option value="user">Collector</option><option value="manager">Manager</option><option value="admin">Admin</option></select></div>
                <button type="submit" class="btn-friendly" style="background:var(--accent); color:white; width:100%">Save Changes</button>
            </form>
        </div>
    </div>
    ...
    <div class="modal-overlay" id="confirmModal">
        <div class="modal-card small">
            ...
            <h3>Delete User?</h3>
            <p>Move this user to the Recycle Bin?</p>
            <div style="display:flex; gap:10px; justify-content:center; margin-top:20px">
                <button class="btn-friendly" onclick="closeModal('confirmModal')">Cancel</button>
                <button class="btn-friendly" id="confirmBtnAction" style="background:var(--red); color:white">Delete</button>
            </div>
        </div>
    </div>
    ...
    <div class="modal-overlay" id="alertModal">
        <div class="modal-card small">
            ...
            <p id="alertMessage"></p>
            <button class="btn-friendly" style="background:var(--text-main); color:white; width:100%; margin-top:20px" onclick="closeModal('alertModal'); location.reload();">Okay</button>
        </div>
    </div>
    ...
    <script>
        ...
        document.getElementById('confirmBtnAction').onclick = () => deleteCallback?.();

        document.getElementById('userForm').onsubmit = async (e) => {
            e.preventDefault();
            const action = document.getElementById('actionType').value;
            const res = await fetch(`manage_user.php?action=${action}`, { method: 'POST', body: new FormData(e.target) });
            const data = await res.json();
            if(data.success) showAlert('Success!', 'User saved successfully.', 'success');
            else showAlert('Error', data.message, 'error');
        };

        async function performDelete(id) {
            const fd = new FormData(); fd.append('id', id);
            const res = await fetch('manage_user.php?action=delete', { method: 'POST', body: fd });
            const data = await res.json();
            if(data.success) showAlert('Deleted!', 'User moved to Recycle Bin.', 'success');
        }

        function showAlert(title, msg, type) {
            document.getElementById('alertTitle').innerText = title;
            document.getElementById('alertMessage').innerText = msg;
            document.getElementById('alertIcon').className = `modal-header-icon ${type}-icon`;
